"""Copied and modified from Huggingface modeling_bert."""
...
from . import MaskedLMOutputBoth
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, knowledges, add_pooling_layer=True):
    ...
    def forward(
        ...
        emb_warmup_alpha: Optional[float] = 1.,
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def __init__(self, config, knowledges):
        ...
        print('Now using GeneCompass model!!!')
    ...
    @add_code_sample_docstrings(
        checkpoint=_CHECKPOINT_FOR_DOC,
        output_type=MaskedLMOutputBoth,
        config_class=_CONFIG_FOR_DOC,
    ...
    def forward(
        ...
        encoder_attention_mask: Optional[torch.Tensor] = None,
        labels: Optional[torch.Tensor] = None,
        labels_values: Optional[torch.Tensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[torch.Tensor], MaskedLMOutputBoth]:
        r"""
        labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
            Labels for computing the masked language modeling loss. Indices should be in `[-100, 0, ...,
        ...
        if labels is not None:
            ...
            if self.use_values:
                ...
                prediction_values = prediction_values.view(-1)[labels.view(-1) != -100]
                labels_values = labels_values.view(-1)[labels.view(-1) != -100]
                # loss_values = self.emb_warmup.alpha * loss_mse(prediction_values, labels_values)
                loss_values = loss_mse(prediction_values, labels_values)
                loss = masked_lm_loss * 0.2 + loss_values * 0.8
            else:
                loss_values = torch.Tensor([0])
                loss = masked_lm_loss
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config, knowledges):
        ...
        self.bert = BertModel(config, knowledges)
        classifier_dropout = (
