...
if [ ${#INPUT_FILES[@]} -eq 0 ]; then
    echo "Usage: $0 <input1.h5ad> [input2.h5ad ...]"
    exit 1
fi
...
for INPUT_FILE in "${INPUT_FILES[@]}"; do
    ...
    for MODEL in "${MODELS[@]}"; do
        ...
        if [ "$MODEL" == "tahoe" ]; then
            NUM_LAYERS=$(python models/get_model_info.py --model "$MODEL" --tahoe_size "$TAHOE_MODEL_SIZE")
        else
            NUM_LAYERS=$(python models/get_model_info.py --model "$MODEL")
        fi
